...
// setup form url.
$form_url       = isset( $_SERVER['REQUEST_URI'] ) ? esc_url_raw( wp_unslash( $_SERVER['REQUEST_URI'] ) ) : '';
$page_var       = isset( $_GET['page'] ) ? sanitize_text_field(  wp_unslash( $_GET['page'] ) ) : '';
...
$fv_regex_html_post_process = isset($_REQUEST['regex_html_post_process']) ? sanitize_text_field( wp_unslash($_REQUEST['regex_html_post_process'] ) ) : $this->get_regex_html_post_process_text();
...
if ( isset( $_POST['options'] ) && isset( $_POST['submit'] ) ) {
    ...
    foreach ( $options as $key => $value ) {
        if ( isset( $_POST['options'][ $key ] ) ) {
            $result = $plugin->set_option( $key, $options[ $key ] = sanitize_option( wp_unslash( $_POST['options'][ $key ] ) ) );
        }
    }
...
if ( isset( $_POST['displayoptions'] ) && isset( $_POST['displayoptions'] ) ) {
	...
	$error = "";
	$plugin->set_wpengine_admin_bar_enabled( sanitize_text_field( wp_unslash( $_POST['wpe-adminbar-enable'] ) ) );
    	if ( empty( $error ) ) {
        	$message = __( "Settings have been successfully updated", PWP_NAME );
    	}
	...
	if ( ! empty( $_POST['wpe-adminbar-roles'] ) ) {
		$roles = array_map( 'sanitize_text_field', wp_unslash( $_POST['wpe-adminbar-roles'] ) );
		$plugin->set_option( 'wpe-adminbar-roles', $roles );
	} else {
...
if ( isset( $_POST['wpe-news-feed-display-options'] )) {
    ...
    $error = "";
    $plugin->set_wpengine_news_feed_enabled( absint(  $_POST['wpe-news-feed-enable'] ) ) ;
    if ( empty( $error ) ) {
        $message = __( "Settings have been successfully updated", PWP_NAME );
    }
}
...
if ( WPE_CDN_DISABLE_ALLOWED && wpe_param( 'cdn-control' ) ) {
	...
	$new_state     = !!sanitize_text_field( wp_unslash( $_REQUEST['cdn-enable'] ) );
...
if ( wpe_param( 'advanced' ) ) {
    ...
    $new_state     = !!sanitize_text_field( wp_unslash( $_REQUEST['rand_enabled'] ) );
    if ( $current_state != $new_state ) {
...
<?php if ( ! empty( $error ) ) : ?>
        <div class="error"><p><?php echo wp_kses_post( $error ); ?></p></div>
<?php endif; ?>

<?php if ( ! empty( $message ) ) : ?>
        <div class="updated fade"><p><?php echo wp_kses_post( $message ); ?></p></div>
<?php endif; ?>
	...
	<h2 class="nav-tab-wrapper">
		...
		<?php if ( ! $plugin->is_legacy_staging_disabled() ) : ?>
	...
	<?php if( $active_tab == 'general'): ?>
        ...
        <?php if($env_ip_pairs) { ?>
            ...
            <?php foreach ($env_ip_pairs as $domain => $ip): ?>
                <blockquote><code><?php echo esc_html( $domain ); ?></code> - <code><?php echo esc_html( $ip ); ?></code></blockquote>
            <?php endforeach; ?>
        <?php } else if ( (defined('WPE_VENDOR') && 'amazon' === WPE_VENDOR) ) { ?>
            <p>Your DNS should be set to CNAME to <code><?php echo esc_html( $site_info->name ); ?>.wpengine.com</code>.</p>
        <?php } else { ?>
        ...
        <?php } ?>
			<p>Your SFTP access (<i>not FTP!</i>) is at hostname <code><?php echo esc_html( $site_info->sftp_host ); ?></code> or IP at <code class="wpe_sftp_ip"><?php echo esc_html( $site_info->sftp_ip ); ?></code> on port <code><?php echo esc_html( $site_info->sftp_port ); ?></code>. You will need to create a Username and Password in order to gain access. This can be <a href="<?php echo esc_url( get_option('wpe-install-userportal','https://my.wpengine.com') ); ?>" target="_blank">created here</a>.</p>
		</div><!--.span-30-->
      		<br class="clear"/>
...
<?php } ?>
				...
				<tr valign="top">
					...
					<td>
				        ...
				        foreach ( $wpe_netdna_domains as $zinfo ) {
				            print("<li><code>" . esc_html( $zinfo['match'] ) . "</code></li>" );
				        }
				        ?></ul>
					</td>
				</tr>
    ...
    <?php } else { ?>
            <p>
                ...
                visit the user portal at <a href="https://my.wpengine.com/installs/<?php echo esc_attr( $site_info->name ); ?>/cdn" target="_blank">https://my.wpengine.com/installs/<?php echo esc_attr( $site_info->name ); ?>/cdn</a>
    ...
    <?php } ?>
				...
				<td>
					...
					<p>
						...
						<?php foreach( $role_names = $roles->get_names() as $role=>$role_name): ?>
							...
							<input name="wpe-adminbar-roles[]" value="<?php echo esc_attr( $role ); ?>" type="checkbox" <?php echo esc_html( $checked ); ?> /> <?php echo esc_html( $role_name ); ?><br/>
						<?php endforeach; ?>
					</p>
				</td>
			</tr>
        ...
        <p>
            [<a href="<?php echo esc_url( $this->get_access_log_url( 'current' ) ); ?>">
                Access Log &mdash; Live Site &mdash; Current
            </a>]<br>
            [<a href="<?php echo esc_url( $this->get_access_log_url( 'previous' ) ); ?>">
                Access Log &mdash; Live Site &mdash; Previous
            </a>]<br>
            [<a href="<?php echo esc_url( $this->get_error_log_url( true ) ); ?>" target="_blank">
                Error Log &mdash; Live Site
            </a>]<br>
            [<a href="<?php echo esc_url( $this->get_error_log_url( false ) ); ?>" target="_blank">
        ...
        <p>
			...
			This is also available in your <a href="<?php echo esc_url( get_option('wpe-install-userportal','https://my.wpengine.com') ); ?>"><?php echo esc_html( get_option('wpe-install-menu_title','WP Engine') ); ?> User Portal</a>.
        </p>
		...
		<table class="form-table">
			...
			<tr valign="top">
				...
				<td>
                    <textarea name="regex_html_post_process" cols="60" rows="5"><?php echo esc_textarea( $fv_regex_html_post_process ); ?></textarea>
					<div class="description">
	...
	<?php elseif($active_tab == 'staging'): ?>
		...
		<?php if ( $snapshot_state['have_snapshot'] ) { ?>
			<div class="alert alert-message alert-success">
				<h3 class="wpe-callout"><i class="wpe-icon-hdd icon-hdd"></i>  Staging Status: <?php echo esc_html( $snapshot_state['status'] ); ?></h3>
			    	<p>
				<?php if ( $snapshot_state['is_ready'] ) { ?>
					Last staging snapshot was taken on <span class="wpe_last_updated_date"><?php echo esc_html( date( "Y-m-d g:i:sa", $snapshot_state['last_update'] + (get_option( "gmt_offset" ) * 60 * 60) ) ); ?></span>. Access it here: <a target="_blank" href="<?php echo esc_url( $snapshot_state['staging_url'] ); ?>"><b><?php echo esc_html( $snapshot_state['staging_url'] ); ?></b></a>
				<?php } else { ?>
					<b>Please wait</b> while the staging area continues to be deployed.  It can take a while!  You can <a href="<?php echo esc_url( $plugin->get_plugin_admin_url('admin.php?page=wpengine-staging') ); ?>">refresh this page</a> to check on its progress.
				<?php } ?>
			    	</p>
			</div>
		<?php } ?>
		...
		<p>
			This takes a snapshot of your blog and copies it to a "staging area" where you can test changes without affecting your live site. There's only one staging area, so every time you click this button the old staging area is lost forever, replaced with a snapshot of your live blog. Both your live and staging areas are backed up daily. You can also create and restore your <?php printf('<a href="https://my.wpengine.com/installs/%s/backup_points" target="_blank">live site</a>', esc_attr( $site_info->name ) ); ?> and <?php printf('<a href="https://my.wpengine.com/installs/%s/backup_points#staging" target="_blank">staging area</a>', esc_attr( $site_info->name ) ); ?> anytime via the WP Engine User Portal.
		</p>

		<p>
			<b>Please note:</b> if you want to access your staging site via SFTP, there is a different username required. You can manage your SFTP users in your <a href="<?php echo esc_url( get_option('wpe-install-userportal','https://my.wpengine.com') ); ?>" target="_blank">User Portal</a>.
		</p>
		...
		<form class="form" id="deploy-from-staging" style="display:none;" action="" method="post">
			...
			<select name="db_mode" style="width:300px;">
			...
			<select name="tables[]" style="width:300px; resize: both;" multiple>
			<?php foreach($tables as $table) : ?>
				<option value="<?php echo esc_attr( $table ); ?>" <?php if('wp_options' == $table) echo 'selected'; ?>><?php echo esc_html( $table ); ?></option>
			<?php endforeach; ?>
			</select>
			</p>
			<p class="clear">
				<label>Email to Notify</label>
				<input type="text" class="text email" name="email" placeholder="<?php echo esc_attr( get_option('admin_email') ); ?>" value="<?php echo esc_attr( get_option('admin_email') ); ?>"/>
			</p>
...
?>
		...
		<?php if($can_push_staging): ?>
			...
			<div class='alert-message alert-error'>
				<br><blockquote>
				<h3>Your Staging Site is Running an Old Version of WordPress (<?php echo esc_html( $staging_status['version'] ); ?>)</h3>
				<p>Your staging site is running an old version of WordPress, WordPress <?php echo esc_html( $staging_status['version'] ); ?>.
                    Before you can deploy your staging site to your live site, you need to
                    <a target="_blank" href="<?php echo esc_url( $staging_status['staging_url'] ); ?>/wp-admin/update-core.php">update WordPress</a>
                    to match your live version, <?php echo esc_html( $production_version ); ?>. Please follow the steps to update WordPress.
                    We recommend you also create a <a href="https://my.wpengine.com/installs/<?php echo esc_html( PWP_NAME );?>/backup_points">backup point</a> before updating.</p>
				</blockquote><br>
			</div>
		<?php endif; ?>
		</form>
...
<div class="wpe-plugin-version">
	<hr/>
	<p>WP Engine Plugin v<?php echo esc_html( WPE_PLUGIN_VERSION ); ?> | <a href="https://my.wpengine.com/support" target="_blank">Support</a></p>
</div>
