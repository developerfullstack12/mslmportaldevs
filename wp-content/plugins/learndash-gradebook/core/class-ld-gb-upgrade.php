...
class LD_GB_Upgrade {
	...
	function get_upgrades() {
		...
		return array(
			...
			'2.0.0' => array( $this, 'upgrade_2_0_0' ),
			'2.0.4' => array( $this, 'upgrade_2_0_4' ),
			'2.1.0' => array( $this, 'upgrade_2_1_0' ),
		);
	}
	...
	function upgrade_2_0_0() {
		...
		$letter_grade_scale = get_option( 'ld_gb_letter_grade_scale', array() );
		$migrated_letter_grade_scale = array();
		...
		$color_grade_scale = get_option( 'ld_gb_grade_color_scale', array() );
		$migrated_color_grade_scale = array();

		// The Grade Cutoff was used as an index in the old version
		foreach ( $color_grade_scale as $grade => $color ) {

			// No need to migrate. The data is already in the right format.
			if ( is_array( $color ) ) break;
	...
	function upgrade_2_0_4() {
		...
		$user_columns = get_option( 'ld_gb_user_columns', array() );

		if ( ! $user_columns ) return;

		$user_columns = array_map( function( $row ) {
