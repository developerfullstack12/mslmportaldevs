...
class LD_GB_AdminPage_Settings {
	...
	function register_settings() {
		...
		register_setting( 'learndash_gradebook-general', 'ld_gb_user_columns' );
		...
		register_setting( 'learndash_gradebook-general', 'ld_gb_gradebook_disable_sorting_by_grades_backend' );
		register_setting( 'learndash_gradebook-general', 'ld_gb_gradebook_non_group_leaders_show_only_group_users' );
		...
		$settings['learndash_gradebook-general'] = array(
			...
			array(
				...
				'args'     => array(
					...
					'options' => array(
						...
						'letter_and_percentage' => __('Letter and Percentage', 'learndash-gradebook' )
					),
				)
			),
			...
			array(
				...
				'label'    => __( 'Grade Rounding Precision', 'learndash-gradebook' ) .
				              ld_gb_get_field_tip( __( 'The number of decimal places a grade will round to. Setting this to -1 will allow you to round to the tens place.', 'learndash-gradebook' ) ),
			...
			array(
				'id' => 'user_columns',
				'label' => __( 'Which columns should be shown for the Gradebook?', 'learndash-gradebook' ) . ld_gb_get_field_tip( __( 'The Overall Grade and Component columns will always be shown', 'learndash-gradebook' ) ),
				...
				'args' => array(
					...
					'fields' => array(
						'column' => array(
							'type' => 'select',
							'args' => array(
								'label' => __( 'Column', 'learndash-gradebook' ),
			...
			array(
				...
				'label'    => __( 'Disable Sorting by Grades for the Backend Gradebook', 'learndash-gradebook' ) .
				              ld_gb_get_field_tip( __( 'If you are having issues loading the Gradebook, try enabling this. This was previously named "Gradebook Safe Mode".', 'learndash-gradebook' ) ),
				'callback' => 'ld_gb_do_field_toggle',
				'section'  => 'main',
				'args'     => array(
					'checked_value' => 'yes',
			...
			array(
				...
				'label'    => sprintf( __( 'Non-Group Leaders only have their own %s Users in their Gradebook', 'learndash-gradebook' ), LearnDash_Custom_Label::get_label( 'group' ) )  .
				              ld_gb_get_field_tip( sprintf( __( 'If you have a lot of Users on your website you may want to enable this. Otherwise, Admins may be unable to access the backend. By default, non-Group Leaders will be calculating grade data for all Students across every %s at once. This setting will not take effect if no %s exist on this site.', 'learndash-gradebook' ), LearnDash_Custom_Label::get_label( 'group' ), LearnDash_Custom_Label::get_label( 'groups' ) ) ),
				'callback' => 'ld_gb_do_field_toggle',
				'section'  => 'main',
				'args'     => array(
					'checked_value' => 'yes',
	...
	function save_letter_grade_scale() {
		...
		// Sort
		usort( $_POST['ld_gb_letter_grade_scale'], function( $a, $b ) {
	...
	function save_grade_scale_colors() {
		...
		// Sort
		usort( $_POST['ld_gb_grade_color_scale'], function( $a, $b ) {
			return $a['grade'] < $b['grade'];
		} );
	...
	function notify_invalid_scales() {
		...
		$grade_scale_search = array_filter( $letter_grade_scale, function( $row ) {
			return $row['grade'] == 0;
		} );
		...
		if ( empty( $grade_scale_search ) ) {
			...
			add_settings_error(
				...
				'',
				__( 'Letter Grade Scale requires a 0% grade to exist. It has been added manually and will show on the next page refresh.', 'learndash-gradebook' ),
				'error'
			);

			// If they deleted everything, the stored value is an empty string. We need to ensure it is an Array for PHP 7.1+ compatibility
			if ( ! is_array( $letter_grade_scale ) ) {
				$letter_grade_scale = array();
			}
		...
		$color_scale_search = array_filter( $grade_color_scale, function( $row ) {
			return $row['grade'] == 0;
		} );
		...
		if ( empty( $color_scale_search ) ) {
			...
			add_settings_error(
				'ld-gb-invalid-scales',
				'',
				__( 'Grade Color Scale requires a 0% grade to exist. It has been added manually and will show on the next page refresh.', 'learndash-gradebook' ),
				'error'
			);
