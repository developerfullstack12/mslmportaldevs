 ...
 * Plugin URI: https://realbigplugins.com/plugins/learndash-gradebook
...
if ( ! class_exists( 'LearnDash_Gradebook' ) ) {
	...
	define( 'LEARNDASH_GRADEBOOK_VERSION', '2.1.1' );
	...
	final class LearnDash_Gradebook {
		...
		private function __construct() {
			...
			add_filter( 'learndash_gradebook_download_id', array( $this, 'set_download_id' ) );
		...
		function register_assets() {
			...
			wp_register_style(
				'ld-gb-report-card',
				LEARNDASH_GRADEBOOK_URI . '/assets/dist/css/report-card.css',
				apply_filters( 'ld_gb_report_card_style_dependencies', array() ),
			...
			wp_register_style(
				...
				apply_filters( 'ld_gb_frontend_gradebook_style_src', LEARNDASH_GRADEBOOK_URI . '/assets/dist/css/frontend-gradebook.css' ),
				apply_filters( 'ld_gb_frontend_gradebook_style_dependencies', array( 'dashicons' ) ),
				defined( 'WP_DEBUG' ) && WP_DEBUG ? time() : LEARNDASH_GRADEBOOK_VERSION
			);
			...
			wp_register_script(
				'ld-gb-frontend-gradebook',
				LEARNDASH_GRADEBOOK_URI . '/assets/dist/js/frontend-gradebook.js',
				apply_filters( 'ld_gb_frontend_gradebook_script_dependencies', array( 'jquery' ) ),
				defined( 'WP_DEBUG' ) && WP_DEBUG ? time() : LEARNDASH_GRADEBOOK_VERSION,
				true
			);
			...
			wp_localize_script(
				...
				apply_filters( 'ld_gb_frontend_gradebook_localize_script', array(
					'l10n' => array(
						'ldFrontendGradebookTableListOptions' => array(
					...
					'i18n' => array(
						'validationError' => __( 'This field is required', 'learndash-gradebook' ),
						'deleteGrade' => __( 'Are you sure you want to delete this grade?', 'learndash-gradebook' ),
					)
				) )
			);
			...
			wp_register_style(
				'ld-gb-admin',
				LEARNDASH_GRADEBOOK_URI . '/assets/dist/css/admin.css',
				array(),
				defined( 'WP_DEBUG' ) && WP_DEBUG ? time() : LEARNDASH_GRADEBOOK_VERSION
			);
			...
			wp_register_script(
				'ld-gb-admin',
				LEARNDASH_GRADEBOOK_URI . '/assets/dist/js/admin.js',
				array( 'jquery' ),
				defined( 'WP_DEBUG' ) && WP_DEBUG ? time() : LEARNDASH_GRADEBOOK_VERSION,
				true
			);
