(function($){$('#gamipress-transfers-change-user, #gamipress-transfers-change-recipient').on('click',function(e){e.preventDefault();var $this=$(this);var field=$this.closest('.cmb-td').find('select');var actions=$this.parent();var field_actions=actions.prev();field.data('current',field.val());if(!field.hasClass('.select2-hidden-accessible')){field.gamipress_select2({ajax:{url:ajaxurl,dataType:'json',delay:250,type:'POST',data:function(params){return{q:params.term,action:'gamipress_get_users',nonce:gamipress_transfers_transfers.nonce,}},processResults:gamipress_select2_users_process_results},escapeMarkup:function(markup){return markup},templateResult:gamipress_select2_users_template_result,theme:'default gamipress-select2',placeholder:'Select an User',allowClear:!0,multiple:!1})}
actions.slideUp();field.slideDown();field.next('.select2').slideDown();field_actions.slideDown()});$('.save-user-select').on('click',function(e){e.preventDefault();var $this=$(this);var field=$this.closest('.cmb-td').find('select');var field_actions=$this.parent();var actions=field_actions.next();var user_id=field.val();var prev_id=field.data('current');if(parseInt(user_id)!==parseInt(prev_id)){var user_data=field.gamipress_select2('data')[0];var user_name_element=$this.closest('.cmb-td').find('h2');var user_email_element=$this.closest('.cmb-td').find('.user-email');if(user_data.user_email!==undefined){user_name_element.html(user_data.display_name);user_email_element.html(user_data.user_email)}else{user_name_element.html(user_name_element.data('original'));user_email_element.html(user_email_element.data('original'))}
...
$.ajax({url:ajaxurl,data:{action:'gamipress_transfers_add_transfer_note',nonce:gamipress_transfers_transfers.nonce,transfer_id:$('#ct_edit_form input#object_id').val(),title:title,description:description},success:function(response){if(response.success){$('.transfer-notes-list tbody').prepend(response.data);$this.closest('#new-transfer-note-fieldset').slideUp();$this.closest('#new-transfer-note-fieldset').prev().slideDown();$('#transfer-note-title').val('');$('#transfer-note-description').val('')}else{notice.find('.error').html(response.data);notice.removeClass('hidden')}
$this.removeClass('disabled')}})});$('#cancel-transfer-note').on('click',function(e){e.preventDefault();$(this).closest('#new-transfer-note-fieldset').slideUp();$(this).closest('#new-transfer-note-fieldset').prev().slideDown();$('#transfer-note-title').val('');$('#transfer-note-description').val('')});$('.transfer-note .row-actions .delete').on('click',function(e){e.preventDefault();var confirmed=confirm('Do you want to remove this transfer note?');var $this=$(this);if(confirmed){$this.closest('.transfer-note').fadeOut();$.ajax({url:ajaxurl,data:{action:'gamipress_transfers_delete_transfer_note',nonce:gamipress_transfers_transfers.nonce,transfer_note_id:$this.data('transfer-note-id'),},success:function(response){if(response.success){$this.closest('.transfer-note').remove()}else{$this.closest('.transfer-note').fadeIn()}}})}})})(jQuery)