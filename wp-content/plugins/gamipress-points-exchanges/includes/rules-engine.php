...
/**
 * Checks if an user is allowed to work on a given requirement related to a specific minimum amount of points to be exchanged
 *
 * @since  1.0.0
 *
...
function gamipress_points_exchanges_user_has_access_to_achievement( $return = false, $user_id = 0, $requirement_id = 0, $trigger = '', $site_id = 0, $args = array() ) {
    ...
    if( $trigger === 'gamipress_points_exchanges_new_points_exchange' ) {
        ...
        $required_type = absint( get_post_meta( $requirement_id, '_gamipress_points_exchanges_points_type', true ) );
        ...
        $required_amount = absint( get_post_meta( $requirement_id, '_gamipress_points_exchanges_points_amount', true ) );
