...
if ( ! class_exists( 'LearnDash_PowerPack_Remove_Video_Progression_Cookie_On_Lesson_Completion', false ) ) {
	/**
	 * LearnDash_PowerPack_Remove_Video_Progression_Cookie_On_Lesson_Completion Class.
	 */
	class LearnDash_PowerPack_Remove_Video_Progression_Cookie_On_Lesson_Completion {
		...
		public function __construct() {
			...
			if ( if_current_class_is_active( $this->current_class ) === 'active' ) {
				add_action( 'learndash_lesson_completed', [ $this, 'learndash_lesson_completed_func' ] );
			}
		}

		/**
		 * Remove the video cookie on lesson complete.
		 *
		 * @param array $args The array of arguments.
		 */
		public function learndash_lesson_completed_func( $args = [] ) {
			$option_active_status = if_current_class_is_active( $this->current_class );
			if ( 'active' !== $option_active_status ) {
				return;
			}
			$this->learndash_remove_video_cookie( $args['lesson']->ID, $args['course']->ID, $args['user']->ID );
		}
		...
		public function learndash_powerpack_class_details() {
			...
			$class_title       = esc_html__( 'Remove Video Progression Cookie on Lesson Completion', 'learndash-powerpack' );
			$class_description = '';
			$deprecated        = esc_html__( 'Setting Deprecated due the functionality is already on LearnDash Core', 'learndash-powerpack' );
			$deprecated_date   = gmdate( 'Y-m-d', strtotime( '2021-07-29' ) );
	...
	new LearnDash_PowerPack_Remove_Video_Progression_Cookie_On_Lesson_Completion();
}
